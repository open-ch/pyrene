...
import { Filters, MultiselectOption, SingleSelectOption } from '../Filter/types';
...
const baseArgs: TableProps<Row> = {
  actions: [
    ...
    },
    {
      ...
      callback: (keys) =>
        alert(keys.map((key) => friends[key as number].age).reduce((v, acc) => acc + v, 0)),
      icon: 'add',
      active: 'multi',
    },
  ],
...
const TemplateFilterable: Story<TableProps<Row>> = ({ filterValues, ...args }) => {
    ...
    updateArgs({
      data:
        filters && Object.keys(filters).length > 0
          ? friends
              .filter((row) => !filters.name || row.name.includes(filters.name as string))
              .filter((row) => {
  ...
  return <Table<Row> {...args} onFilterChange={onFilterChange} filterValues={filterValues} />;
};
