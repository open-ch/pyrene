...
export default class FilterBar extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      unAppliedFilters: this._createUnappliedFilters(props),
    };
  }
  ...
  toggleFilterPopover = () => {
    if (!this.state.displayFilterPopover) {
      this.setState({ unAppliedFilters: this._createUnappliedFilters(this.props) });
    }
  ...
  filterDidChange = (value, negated, key) => {
    this.setState((prevState) => ({
      unAppliedFilters: {
        ...
        negatedKeys: this._getNegatedFilterKeysForChange(prevState, negated, key),
      },
    }));
  };
  ...
  _createUnappliedFilters = (props) => {
    const negatedFiltersKeys = this._getNegatedFilterKeys(props, Object.keys(this._getValidFilterEntries(props.filterValues)));
    return {
  ...
  _getNegatedFilterKeys = (props, filteredKeys) => props.filters.filter((filter) => filter.negated && filteredKeys.includes(filter.id))
  ...
  _getNegatedFilterKeysForChange = (prevState, negated, key) => {
  ...
  _getValidFilterEntries = (filterValues) => Object.entries(filterValues)
    .filter(([key, value]) => value !== null) // eslint-disable-line no-unused-vars
    .reduce((merged, [key, value]) => ({ ...merged, [key]: value }), {});

  applyFilter = () => {

    const filtered = this._getValidFilterEntries(this.state.unAppliedFilters.values);
  ...
  getFilterTags() {
    ...
    if (filterValues) {
      ...
      const tags = Object.entries(filterValues).map(([key, value]) => {
        ...
        switch (filter.type) {
          case 'text':
            return <FilterTag key={filter.id} filterLabel={filter.label} filterText={value} negated={this.props.isNegationEnabled && filter.negated} onClose={() => this.onFilterTagClose(filter)} />;
          case 'singleSelect':
            return <FilterTag key={filter.id} filterLabel={filter.label} filterText={value.label} negated={this.props.isNegationEnabled && filter.negated} onClose={() => this.onFilterTagClose(filter)} />;
          case 'multiSelect':
            if (value.length > 0) {
              return <FilterTag key={filter.id} filterLabel={filter.label} filterText={value.map((option) => option.label).join('; ')} negated={this.props.isNegationEnabled && filter.negated} onClose={() => this.onFilterTagClose(filter)} />;
            }
            break;
          default:
    ...
    return (
      <div styleName="filter">
        <FilterPopoverButton
          ...
          isNegationEnabled={this.props.isNegationEnabled}
