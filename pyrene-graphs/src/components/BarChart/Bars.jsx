...
import { Bar, chartConstants, scaleUtils } from 'tuktuktwo';
...
const Bars = (props) => {
  ...
  const scale = scaleUtils.scaleCustomBand(props.values, 0, size, paddingInner, paddingOuter);
  return (
    <g>
      {props.values.map((d, i) => (
        ...
        key={`bar-${i}`} // eslint-disable-line
          ...
          x={props.direction === 'horizontal' ? left : scale(d) + left}
          y={props.direction === 'horizontal' ? scale(d) : chartConstants.marginMaxValueToBorder}
        />
      ))}
    </g>
  );
